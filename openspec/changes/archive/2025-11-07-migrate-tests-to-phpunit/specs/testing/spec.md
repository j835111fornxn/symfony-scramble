...
## Notes on Removed Pest Features

This change replaces Pest with PHPUnit. Previously removed Pest features include:
- Pest's `it()` and `test()` functional testing API (now using PHPUnit test methods)
- Pest's `expect()` assertions API (now using PHPUnit assertions)
- Pest's `uses()` trait declaration (now using class-based inheritance)
- Pest's `beforeEach()` and `afterEach()` hooks (now using `setUp()` and `tearDown()`)
- Pest's custom expectations (now using custom assertion methods)
